...
<%--	<jsp:forward page="users.do?mth=findAll"></jsp:forward>--%>
	<script type="text/javascript">
		window.location.href="users.do?mth=findAll";
	</script>
</c:if>
<body>
					...
					<tr>
						...
						<td class="w1 c">
							<c:choose>
								...
								<c:when test="${user.state==0}">禁用</c:when>
							</c:choose>
						</td>
						<td class="w1 c">
							<c:choose>
								<c:when test="${user.state==1}">
									<a href="users.do?mth=modifyState&uid=${user.userId}&state=0">禁用</a>
								</c:when>
								<c:when test="${user.state==0}">
									<a href="users.do?mth=modifyState&uid=${user.userId}&state=1">恢复</a>
								</c:when>
							</c:choose>
					</tr>
				</c:forEach>
