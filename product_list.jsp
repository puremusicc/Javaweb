		...
		<div style="width:380px;margin:0 auto;margin-top:50px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				<c:if test="${page.currentPage>1}">
					<li class="disabled">
						<a href="GoodsServlet?mth=findByTid&tid=${tid}&curr=${page.currentPage-1}" aria-label="Previous">
				...
				<c:forEach begin="1" end="${page.totalPage}" var="num">
					<li <c:if test="${num==page.currentPage}"> class="active" </c:if> >
						<a href="GoodsServlet?mth=findByTid&tid=${tid}&curr=${num}">${num}</a>
					</li>
				</c:forEach>
				<c:if test="${page.currentPage< page.totalPage}">
					<li>
						<a href="GoodsServlet?mth=findByTid&tid=${tid}&curr=${page.currentPage+1}" aria-label="Next">
